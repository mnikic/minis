...
// Global config for signal handlers (if needed later)
...
void parse_args (int argc, char *argv[], MinisConfig *cfg) {
    // Set defaults
    ...
    while ((opt = getopt_long(argc, argv, "p:dvh", long_options, &option_index)) != -1) {
        switch (opt) {
            case 'p':
...
void daemonize_process(void) {
    ...
    // 3. Catch, ignore and handle signals (TODO)
    signal(SIGCHLD, SIG_IGN);
    signal(SIGHUP, SIG_IGN);

    // 4. Fork off for the second time (prevent re-acquiring TTY)
    pid = fork();
    if (pid < 0) exit(EXIT_FAILURE);
    if (pid > 0) exit(EXIT_SUCCESS);

    // 5. Change file mode mask
    umask(0);

    // 6. Change working directory to root (so we don't lock folders)
    ...
    if (file_desc != -1) {
        ...
        if (file_desc > 2) close(file_desc );
    }
}

void handle_signal(int sig) {
    if (sig == SIGINT || sig == SIGTERM) {
        // This is where you'd call save_data() if you add persistence!
