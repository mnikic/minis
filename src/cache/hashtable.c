...
hm_help_resizing (HMap *hmap)
{
    ...
    while (nwork < k_resizing_work && hmap->ht2.size > 0) {
        ...
        for (;;) {
            ...
            size_t exist_dist = probe_distance(n_entry->hcode_cached, n_pos, hmap->ht1.mask);
...
h_delete (HTab *htab, HTabEntry *slot)
{
    ...
    for (;;) {
        ...
        size_t dist = probe_distance(next_entry->hcode_cached, next_pos, htab->mask);
...
hm_destroy (HMap *hmap)
{
    ...
    if (hmap->ht1.tab) free(hmap->ht1.tab);
    if (hmap->ht2.tab) free(hmap->ht2.tab);
    memset(hmap, 0, sizeof(*hmap));
}
