...
static bool
load_one_entry (FILE *file_ptr, uint64_t now_us, Cache *cache, uint32_t *crc,
		bool *is_eof)
{
  uint64_t expire_net;
...
bool
cache_save_to_file (const Cache *cache, const char *filename, uint64_t now_us)
{
...
bool
cache_load_from_file (Cache *cache, const char *filename, uint64_t now_us)
{
  ...
  while (true)
    {
      bool is_eof = false;
      if (!load_one_entry (file_ptr, now_us, cache, &running_crc, &is_eof))
	{
