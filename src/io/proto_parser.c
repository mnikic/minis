...
{
  if (unlikely(limit < 4)) return -1;

  size_t pos = 0;
  if (unlikely(buf[pos] != '*')) return -1;
  pos++;
...
{
  if (reqlen < 4) return VALIDATE_TOO_SHORT;

  uint32_t num;
  memcpy(&num, req, 4);
  num = ntohl (num);

  if (num > K_MAX_ARGS) return VALIDATE_TOO_MANY_ARGS;
  if (num < 1) return VALIDATE_TOO_FEW_ARGS;
...
parse_resp_arguments (ProtoRequest *req)
{
  ...
  if (unlikely(buf[pos] != '*')) return PARSE_BAD_PROTOCOL;
  pos++;

  int64_t count = 0;
  read_resp_int (buf, limit, &pos, &count); // Result assumed valid from scan

  if (unlikely(count > K_MAX_ARGS)) return PARSE_OUT_OF_MEMORY;
