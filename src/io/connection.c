...
// Writes data to the circular buffer and appends it to the current slot's IOVEC.
// Returns false if no space.
bool
conn_append_to_wbuf (Conn *conn, ResponseSlot *slot, void *data, uint32_t len)
{
  if (len == 0)
    return true;

  // 1. Check Space
  if (conn_wbuf_free_space (conn) < len)
    {
      return false;		// Caller must handle "Buffer Full"
    }

  // 2. Sanity Check: IOV limit
  if (slot->iov_cnt >= K_IOV_PER_SLOT)
    {
      // This happens if you try to append 50 different tiny strings to one slot.
      // For Redis protocol, we usually just do Header + Body + CRLF, so 3-4 is enough.
      return false;
    }

  uint32_t head = conn->wbuf_head;
  uint32_t size = conn->wbuf_size;

  // 3. Does it wrap around?
  if (head + len > size)
    {
      // --- WRAP AROUND CASE ---
      // We need TWO iovec entries: one for the end, one for the start.

      if (slot->iov_cnt + 1 >= K_IOV_PER_SLOT)
	return false;		// Need 2 slots

      uint32_t part1_len = size - head;
      uint32_t part2_len = len - part1_len;

      // Copy Part 1 (End of buffer)
      memcpy (conn->wbuf + head, data, part1_len);
      slot->iov[slot->iov_cnt].iov_base = conn->wbuf + head;
      slot->iov[slot->iov_cnt].iov_len = part1_len;
      slot->iov_cnt++;

      // Copy Part 2 (Start of buffer)
      memcpy (conn->wbuf, (uint8_t *) data + part1_len, part2_len);
      slot->iov[slot->iov_cnt].iov_base = conn->wbuf;
      slot->iov[slot->iov_cnt].iov_len = part2_len;
      slot->iov_cnt++;

      conn->wbuf_head = part2_len;
    }
  else
    {
      // --- CONTIGUOUS CASE ---
      memcpy (conn->wbuf + head, data, len);

      slot->iov[slot->iov_cnt].iov_base = conn->wbuf + head;
      slot->iov[slot->iov_cnt].iov_len = len;
      slot->iov_cnt++;

      conn->wbuf_head = (head + len) % size;	// Simple advance
    }

  // 4. Update Accounting
  slot->wbuf_bytes_used += len;
  slot->total_len += len;
